import { HardDrive, Home, Laptop, MessageSquare, Settings, Smartphone } from "lucide-react";
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 px-2 py-1.5 flex items-center justify-around shadow-lg">
      ...
      <button
        ...
        className={`relative flex flex-col items-center gap-1 py-1.5 px-3 rounded-xl transition-colors ${
          activeTab === "devices"
            ? "text-indigo-600 dark:text-indigo-400 font-bold"
            : "text-slate-500 dark:text-slate-400 hover:text-slate-800"
        }`}
      >
        <div className="relative">
          ...
          {connectedCount > 0 && (
            <span className="absolute -top-1 -right-2 px-1 py-0.2 rounded-full bg-emerald-500 text-white text-[9px] font-bold">
              {connectedCount}
            </span>
          )}
        </div>
      ...
      >
        <div className="relative">
          ...
          {unreadTotal > 0 && (
            <span className="absolute -top-1 -right-2 px-1.5 py-0.2 rounded-full bg-rose-500 text-white text-[9px] font-bold animate-pulse">
              {unreadTotal}
            </span>
          )}
        </div>
